...
// 		console.log(key);
		...
		$("#saveId").change(function() { // 체크박스에 변화가 있다면,
			if($("#saveId").is(":checked")) { // ID저장하기 체크했을 때
				...
				setCookie("keypw", $("#sellerPw").val(), 7);
			} else { // ID저장 체크 해제 시
				deleteCookie("key");
				deleteCookie("keypw");
			}
		...
		$("#sellerPw").keyup(function() { // pw 입력할 때
			if($("#saveId").is(":checked")) { // 로그인 정보 저장하기를 체크한 상태라면
				setCookie("keypw", $("#sellerPw").val(), 7); // 7일 동안 쿠키 보관
			}
		});
	});
...
<input style="height: 30px; width: 250px; margin: 10px" type="password" id="sellerPw" name="sellerPw" />
<br>
		...
		if(${chatRoomNo} == noFlag){
			...
			for(var i=0; i<refreshList.length; i++){
				...
				if($("#b"+refreshList[i].chatRoomNo).length<=0){
					var a = "<div id=\"b"+noFlag+"\" onclick=\"location.href='/seller/main?chatRoomNo="+noFlag+"'\" class=\"chat_list\"><a href=\"/seller/main?chatRoomNo="+noFlag+"\">"+noFlag+"번방["+refreshList[i].theOtherParty+"]</a> <div class=\"chat_people\"><div class=\"chat_img\"> <img src=\"https://ptetutorials.com/images/user-profile.png\" alt=\"sunil\"> </div><div class=\"chat_ib\"><p><span class=\"time_date\"> ["+presentDate+"]</span>"+senderId+" : "+result+"</p><div id=\"c"+refreshList[i].chatRoomNo+"\"></div></div></div></div>"
